...
def compute_avglogL(X,y,beta):
    ...
    sumlogL = 0
    for i in range(n):
        sumlogL += y[i] * np.dot(X[i].transpose(), beta) - np.logaddexp(0, np.dot(X[i].transpose(), beta))
    avglogL = sumlogL/n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        for j in range(p):
            for i in range(n):
                z = np.dot(beta.transpose(), train_x[i])
                if ( z < 0):
                    theta[j] += train_x[i][j] * (train_y[i] - (np.exp(z)/(1+np.exp(z))))
                else:
                    theta[j] += train_x[i][j] * (train_y[i] - 1/(1 + np.exp(-z)))
        beta = beta - np.dot(lr,(theta-np.dot(lamb, beta)))
        ...
        for j in range(p):
            for n1 in range(p):
                for i in range(n):
                    z = np.dot(beta.transpose(), train_x[i])
                    if (z<0):
                        hessian[j][n1] -= train_x[i][j]*train_x[i][n1] * (np.exp(z)/(1+np.exp(z))) * (1-(np.exp(z)/(1+np.exp(z))))
                    else:
                        hessian[j][n1] -= train_x[i][j]*train_x[i][n1] * (1/(1 + np.exp(-z))) * (1 - (1/(1 + np.exp(-z))))

        beta -= np.dot(np.linalg.inv(hessian + 0.00001 * np.random.rand(p,p)), theta)
